...
import { fetchProductsWithParams } from "@/utils/api";
...
const Products = async ({ slug }: any) => {
  ...
  const products = await fetchProductsWithParams({
    price: slug?.searchParams?.price || 3000,
    rating: Number(slug?.searchParams?.rating) || 0,
    stockMode,
